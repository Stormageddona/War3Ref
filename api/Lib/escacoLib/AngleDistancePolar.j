...
library Polar
    ...
    struct Polar extends array
        ...
        static method RadUTA takes unit u, real dist, real angle returns nothing
            call DzSetUnitPosition(u,GetUnitX(u)+ dist * Cos(angle),GetUnitY(u)+ dist * Sin(angle))
        endmethod
        
        static method UTA takes unit u, real dist, real angle returns nothing
            call DzSetUnitPosition(u,GetUnitX(u)+ dist * Cos(angle * bj_DEGTORAD),GetUnitY(u)+ dist * Sin(angle * bj_DEGTORAD))
        endmethod
